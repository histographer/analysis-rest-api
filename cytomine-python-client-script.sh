...
# From https://packagecloud.io/install/repositories/cytomine-uliege/Cytomine-python-client/script.python.sh
...
virtualenv_check ()
{
  if [ -z "$VIRTUAL_ENV" ]; then
    echo "Detected VirtualEnv: Please visit https://packagecloud.io/cytomine-uliege/Cytomine-python-client/install#virtualenv"
  fi
}
...
abort_already_configured ()
{
  if [ -e "$HOME/.pip/pip.conf" ]; then
    if grep -q "cytomine-uliege/Cytomine-python-client" "$HOME/.pip/pip.conf"; then
...
pip_extra_url ()
{
  printf "[global]\nextra-index-url=https://packagecloud.io/cytomine-uliege/Cytomine-python-client/pypi/simple\n"
}
